...
interface AuthState {
  ...
  setBackendRegistrationCallback: (
    callback: (email: string, name: string, isAdmin: boolean) => Promise<void>,
  ) => void;
}
...
const getUsers = (): StoredUser[] => {
  ...
  const adminIndex = users.findIndex(
    (u: StoredUser) => u.email.toLowerCase() === ADMIN_EMAIL.toLowerCase(),
  );
  if (adminIndex === -1) {
...
let backendRegistrationCallback:
  | ((email: string, name: string, isAdmin: boolean) => Promise<void>)
  | null = null;
    ...
    (set, get) => ({
        ...
        try {
          ...
          if (backendRegistrationCallback) {
            try {
              ...
              await backendRegistrationCallback(
                user.email,
                user.name,
                isGuaranteedAdmin,
              );
              console.log(
                "[useAuth] User profile registered with backend successfully",
              );
            } catch (error) {
              ...
              if (isGuaranteedAdmin) {
                console.warn(
                  "[useAuth] Admin user failed initial backend sync. Backend will recognize admin email on next operation.",
                );
              }
            }
          }
      ...
      signup: async (
        ...
        try {
          ...
          if (backendRegistrationCallback) {
            try {
              console.log(
                "[useAuth] Registering new user profile with backend...",
              );
              await backendRegistrationCallback(
                newUser.email,
                newUser.name,
              ...
              console.error(
                "[useAuth] Failed to register user profile with backend:",
                error,
              );
            }
          }
      ...
      isAdmin: () => {
        ...
        return (
          ...
          (state.user?.role === "admin" || isGuaranteedAdmin)
        );
      },
